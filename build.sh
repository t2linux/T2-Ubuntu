...
do
  ...
  echo >&2 "===]> Info: Build Ubuntu Mantic... "
  /bin/bash -c "
    ROOT_PATH=${ROOT_PATH} \\
  ...
  cd "${ROOT_PATH}"
  zip -s 1500m "${ROOT_PATH}/output/ubuntu-23.10-${KERNEL_VERSION}-${ALTERNATIVE}.zip" "${ROOT_PATH}/ubuntu-23.10-${KERNEL_VERSION}-${ALTERNATIVE}.iso"
done
## Calculate sha256 sums of built ISO
sha256sum "${ROOT_PATH}"/*.iso >"${ROOT_PATH}/output/sha256-ubuntu-23.10"
