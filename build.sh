...
for ALTERNATIVE in t2-questing
do
  echo >&2 "===]> Info: Start building ${ALTERNATIVE}... "

  echo >&2 "===]> Info: Build Ubuntu Questing... "
  /bin/bash -c "
    ROOT_PATH=${ROOT_PATH} \\
  ...
  echo >&2 "===]> Info: Build Image Questing... "
  /bin/bash -c "
    ROOT_PATH=${ROOT_PATH} \\
    WORKING_PATH=${WORKING_PATH} \\
    CHROOT_PATH=${CHROOT_PATH}_${ALTERNATIVE} \\
  ...
  cd "${ROOT_PATH}"
  split -b 900M -x "${ROOT_PATH}/ubuntu-25.10-${KERNEL_VERSION}-${ALTERNATIVE}.iso" "${ROOT_PATH}/output/ubuntu-25.10-${KERNEL_VERSION}-${ALTERNATIVE}.iso."
done
## Calculate sha256 sums of built ISO
sha256sum "${ROOT_PATH}"/*.iso >"${ROOT_PATH}/output/sha256-ubuntu-25.10"
